...
export default function ClarityPage() {
  ...
  async function run(ev) {
    ev.preventDefault();             // <-- prevents navigation
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-10">
      ...
      <form onSubmit={run} className="flex gap-3 mb-6">
        ...
        />
        <button type="submit" disabled={loading} className="btn-pill bg-black text-white">
          {loading ? "Analyzing…" : "Run"}
        </button>
      </form>

      {err && <div className="mb-6 rounded-lg border border-red-300 bg-red-50 p-4 text-red-800">{err}</div>}
